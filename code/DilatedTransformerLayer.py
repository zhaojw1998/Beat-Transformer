...
class DilatedMultiheadSelfAttentionWithRelativePositionalEmbedding(nn.Module):
    ...
    def forward(self, query, key, value, layer=0):
        ...
        #k = self.kv_roll(k, layer, padding_value=0)  #(batch, num_head, time, attn_len, head_dim)
        #v = self.kv_roll(v, layer, padding_value=0)  #(batch, num_head, time, attn_len, head_dim)
