...
NLAYER=9
#directories
DATASET_PATH = '/data1/zhaojw/dataset/linear_spectrogram_data_11k.npz'
ANNOTATION_PATH = '/data1/zhaojw/dataset/beat_annotation.npz'
MODEL_PATH = "/data1/zhaojw/shell_beat_transformer_final/01_MS-tempo-rpe-EDT-9layers-attn_len_5_multi_asymmetric-RAdam_look_ahead-patience_2-rwc_enriched/Fold_6/model/trf_param_019.pt"
DATA_TO_LOAD = ['gtzan']   #'carnetic', 'harmonix'   # ballroom,  'gtzan', 'hainsworth', 'smc'
TEST_ONLY = ['gtzan']
DEMO_SAVE_ROOT = '/data1/zhaojw/demo_save/'
...
with torch.no_grad():
    for idx, (dataset_key, data, beat, downbeat, tempo, root) in tqdm(enumerate(loader), total=len(loader)):
        ...
        #gt beat
        beat_gt = torch.nonzero(beat[0]>.5)[:, 0].detach().numpy() / (FPS/DS_RATIO)
        dnb_beat_pred = beat_tracker(beat_pred)

        downbeat_gt = torch.nonzero(downbeat[0]>.5)[:, 0].detach().numpy() / (FPS/DS_RATIO)
        ...
        #fig = ax.get_figure()
        print('saving...')
        plt.savefig('final_model_demixed_multi_attn_attention_head_full.pdf', format='pdf', dpi=1200)
