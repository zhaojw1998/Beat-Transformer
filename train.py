...
FOLD = int(sys.argv[2])
GPU = int(sys.argv[3])
PROJECT_NAME = 'MS-tempo-rpe-EDT-9layers-attn_len_5_multi_asymmetric-RAdam_look_ahead-patience_2-rwc_enriched'
...
DATA_TO_LOAD = ['ballroom', 'gtzan', 'hainsworth', 'smc', 'harmonix', 'rwc', 'carnetic']   #musicnet
TEST_ONLY = ['gtzan']

SAVE_PATH = '/data1/zhaojw/shell_beat_transformer_final'
if not os.path.exists(SAVE_PATH + '/debug'):
    os.makedirs(SAVE_PATH + '/debug')
SAVE_PATH = f'/data1/zhaojw/shell_beat_transformer_final/{str(GPU).zfill(2)}_{PROJECT_NAME}'
...
if DEBUG_MODE:
    ...
    SAVE_PATH = '/data1/zhaojw/shell_beat_transformer_final/debug'
...
beat_tracker = madmom.features.beats.DBNBeatTrackingProcessor(min_bpm=55.0, max_bpm=215.0, fps=FPS/DS_RATIO, transition_lambda=10, threshold=0.05)
downbeat_tracker = madmom.features.downbeats.DBNDownBeatTrackingProcessor(beats_per_bar=[3, 4], min_bpm=55.0, max_bpm=215.0, fps=FPS/DS_RATIO, transition_lambda=10)
...
def evaluate(model, val_loader, loss_func, loss_tempo, epoch, device):
    ...
    with torch.no_grad():
        for idx, (dataset_key, data, beat_gt, downbeat_gt, tempo_gt) in tqdm(enumerate(val_loader), total=num_batch):
            ...
            beat_acc_DBN = infer_beat_with_DBN(pred[:, :, 0], beat_gt, beat_tracker, FPS/DS_RATIO)
            ...
            downbeat_DBN_acc = infer_downbeat_with_DBN(pred[:, :, 0], pred[:, :, 1], downbeat_gt, downbeat_tracker, FPS/DS_RATIO)
            if not dataset_key[0] == 'smc':
